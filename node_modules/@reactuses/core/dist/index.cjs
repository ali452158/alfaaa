...
const useAsyncEffect = (effect, cleanup = noop$1, deps)=>{
    ...
    React.useEffect(()=>{
        const execute = ()=>_async_to_generator$9(function*() {
                if (!mounted()) {
                    return;
                }
                yield effect();
...
const useCookie = (key, options = defaultOptions$1, defaultValue)=>{
    ...
    const updateCookie = React.useCallback((newValue)=>{
        ...
        window.dispatchEvent(new CustomEvent(SAME_TAB_COOKIE_EVENT, {
            detail: {
                key,
                value
            }
...
function dispatchSameTabStorage(detail) {
    window.dispatchEvent(new CustomEvent(SAME_TAB_STORAGE_EVENT, {
        detail
    }));
}
    ...
    React.useEffect(()=>{
        ...
        try {
            ...
            if ((raw === null || raw === undefined) && data !== null) {
                ...
                storage == null ? void 0 : storage.setItem(key, written);
                lastRawRef.current = undefined;
                notifyRef.current == null ? void 0 : notifyRef.current.call(notifyRef);
...
const useEyeDropper = ()=>{
    ...
    const open = React.useCallback((options = {})=>_async_to_generator$8(function*() {
            if (!isSupported) {
                return {
                    sRGBHex: ''
                };
...
const useFileDialog = (options = defaultOptions$1)=>{
    ...
    const open = (localOptions)=>_async_to_generator$7(function*() {
            if (!inputRef.current) {
                return;
            }
            const _options = _extends$5({}, DEFAULT_OPTIONS, options, localOptions);
...
const useInfiniteScroll = (target, onLoadMore, options = defaultOptions$1)=>{
    ...
    const state = useScroll(target, _extends$4({}, options, {
        offset: _extends$4({
    ...
    useUpdateEffect(()=>{
        const element = getTargetElement(target);
        const fn = ()=>_async_to_generator$6(function*() {
...
const useMediaDevices = (options = {})=>{
    ...
    const ensurePermissions = React.useCallback(()=>_async_to_generator$5(function*() {
            if (!isSupported) {
                return false;
            }
...
function buildAudioConstraints(deviceId, extra) {
    const merged = _extends$3({}, DEFAULT_AUDIO_CONSTRAINTS, extra || {});
    if (deviceId) merged.deviceId = {
        exact: deviceId
    };
    return merged;
}
...
function resolveMimeType(preferred) {
    const MR = (typeof window !== 'undefined' ? window.MediaRecorder : undefined) || (typeof globalThis !== 'undefined' ? globalThis.MediaRecorder : undefined);
...
const useMicrophone = (options = {})=>{
    ...
    const isSupported = useSupported(()=>typeof navigator !== 'undefined' && !!navigator.mediaDevices && typeof navigator.mediaDevices.getUserMedia === 'function');
    ...
    const [recorderState, setRecorderState] = React.useState(null);
    const [resolvedMime, setResolvedMime] = React.useState(null);
    ...
    const lastLevelEmitRef = React.useRef(-Infinity);
    const levelIntervalRef = React.useRef(levelInterval);
    React.useEffect(()=>{
    ...
    const buildAudioGraph = React.useCallback((s)=>{
    ...
    const teardownAudioGraph = React.useCallback(()=>{
        var _sourceRef_current;
        cancelRaf();
        (_sourceRef_current = sourceRef.current) == null ? void 0 : _sourceRef_current.disconnect();
    ...
    const stopRecorderIfActive = React.useCallback(()=>{
    ...
    const attachTrackEndedListeners = useEvent((s)=>{
    ...
    const start = useEvent(()=>_async_to_generator$4(function*() {
            if (!isSupported) {
    ...
    useDeepCompareEffect(()=>{
        ...
        (()=>_async_to_generator$4(function*() {
                try {
                    const audio = buildAudioConstraints(deviceId, constraints);
                    const ns = yield navigator.mediaDevices.getUserMedia({
                        audio
                    });
    ...
    useUnmount(()=>{
        ...
        stop();
        (_audioContextRef_current = audioContextRef.current) == null ? void 0 : _audioContextRef_current.close().catch(()=>{});
    ...
    const startRecording = useEvent(()=>{
        ...
        const recorder = new MR(streamRef.current, mime ? {
            mimeType: mime
        } : undefined);
        ...
        recorder.onstop = ()=>{
            const finalBlob = chunksRef.current.length ? new Blob(chunksRef.current, {
                type: mime || chunksRef.current[0].type
            }) : null;
            if (finalBlob) {
            ...
            stopResolverRef.current == null ? void 0 : stopResolverRef.current.call(stopResolverRef, finalBlob);
